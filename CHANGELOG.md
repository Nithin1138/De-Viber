...
### Fixed
- **Conditional .npmrc generation**: Removed the unconditional `.npmrc` generation call that still executed on every transform run. Now, `.npmrc` is strictly generated/modified only when a real peer-dependency conflict is detected via the `DEP_PEER_CONFLICT_001` finding.
...
### Fixed
- **Auto-commit after transform**: After Docker verification passes, the CLI now automatically runs `git add .` and commits the transformed changes with a clear commit message. Previously, files were only modified on disk but never committed, so there was no visible diff between the backup branch and `main`, and remote platforms (Vercel, Railway) would never see the cleaned-up code. Users are also told to `git push` afterward.
